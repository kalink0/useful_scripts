...
# CREATION DATE:  2019/09/074
#
# LICENSE:        CC0-1.0
#
# SOURCE:         https://github.com/kalink0/useful_scripts/android
#
# TITLE:          live_info_android_windows.ps1
#
...
# rooted or not
$ID = adb shell id
$ID_SU = adb shell su -c id

if ((${ID} -match "root") -or (${ID_SU} -match "root")) {
	$ROOTED = "TRUE"
} else {
	$ROOTED = "FALSE"
}
...
# IMEI
adb shell service call iphonesubinfo 1 | %{ $_.Split("'")[1];} |  %{ $_ -replace "\D", ""} | ForEach-Object { $IMEI1 += $_ }
adb shell service call iphonesubinfo 3 | %{ $_.Split("'")[1];} |  %{ $_ -replace "\D", ""} | ForEach-Object { $IMEI2 += $_ }
if ( $IMEI1 -eq $IMEI2 ) {
  $IMEI2 = ""
}
...
$SUM_FILE = -join (${DEVICE_ID}, '_summary.txt')
