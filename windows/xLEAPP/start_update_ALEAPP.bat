...
::
:: TITLE:          start_update_ALEAPP.bat
::
:: DESCRIPTION:    Setup, update and launch ALEAPP (CLI and GUI)
::
::
:: KNOWN RESTRICTIONS:
...
::		   ALEAPP will be stored in same directory, subfolder ALEAPP
::
...
set "REPO_URL=https://github.com/abrignoni/ALEAPP.git"
set "OFFLINE=0"

title ALEAPP Setup and Launcher

echo.
echo  ============================================================
echo   ALEAPP Setup and Launcher
echo  ============================================================
echo.
...
:: ------------------------------------------------------------
:: 2. Clone or Update ALEAPP  (INTERNET REQUIRED)
:: ------------------------------------------------------------
echo [2/4] ALEAPP source code...

if exist "%ALEAPP_DIR%\.git" goto :repo_exists
if exist "%ALEAPP_DIR%\aleappGUI.py" goto :repo_manual
goto :repo_clone
...
echo.
set /p DO_UPDATE=" Do you want UPDATE ALEAPP? (Y/N): "
...
echo        ALEAPP folder found (not a git repository). Skipping update check.
set "OFFLINE=1"
goto :repo_done
...
git clone "%REPO_URL%" "%ALEAPP_DIR%"
if errorlevel 1 (
    echo.
    echo  [ERROR] Failed to clone the ALEAPP repository.
...
if "%OFFLINE%"=="1" (
    ...
    if exist "%ALEAPP_DIR%\requirements.txt" (
        echo        Upgrading pip...
        python -m pip install --upgrade pip --quiet
        echo        Installing / updating packages from requirements.txt...
        pip install -r "%ALEAPP_DIR%\requirements.txt"
        if errorlevel 1 (
            echo.
...
:: ------------------------------------------------------------
echo  ============================================================
echo   ALEAPP is ready!
echo  ============================================================
echo.
echo   What would you like to do?
echo.
echo   [1] Launch ALEAPP GUI  (aleappGUI.py)
echo   [2] Launch ALEAPP CLI  (aleapp.py)
echo   [3] Exit
echo.
...
echo.
if exist "%ALEAPP_DIR%\aleappGUI.py" (
    python "%ALEAPP_DIR%\aleappGUI.py"
) else (
    echo  [ERROR] aleappGUI.py not found in %ALEAPP_DIR%
...
:launch_cli
echo.
echo  Launching ALEAPP CLI...
echo.
if exist "%ALEAPP_DIR%\aleapp.py" (
    python "%ALEAPP_DIR%\aleapp.py" -h
    echo.
    echo  Tip: Run aleapp.py directly with your desired arguments.
    echo  Example: python aleapp.py -t android -i ^<input_path^> -o ^<output_path^>
) else (
    echo  [ERROR] aleapp.py not found in %ALEAPP_DIR%
    echo  The repository structure may have changed.
)
goto :done
